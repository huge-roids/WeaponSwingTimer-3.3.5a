...
addon_data.config.InitializeVisuals = function()
    ...
    panel.logo:SetTexture('Interface\\AddOns\\WeaponSwingTimer\\Images\\LandingPage.blp')
...
addon_data.config.EditBoxFactory = function(g_name, parent, title, w, h, enter_func)
    ...
    -- Disable auto-focus immediately, before anything else below has a chance to error.
...
addon_data.config.SliderFactory = function(g_name, parent, title, min_val, max_val, val_step, func)
    ...
    -- Disable auto-focus immediately (before anything below can error out) so this
    -- edit box can never end up silently holding keyboard focus and blocking WASD/ESC.
    ...
    -- SetObeyStepOnDrag doesn't exist on 3.3.5 (WotLK) - only call it if available.
...
addon_data.config.color_picker_factory = function(g_name, parent, r, g, b, a, text, on_click_func)
    ...
    color_picker.normal:SetTexture(1, 1, 1, 1)
    ...
    color_picker.foreground:SetTexture(r, g, b, a)
...
addon_data.config.IsLockedCheckBoxOnClick = function(self)
    character_player_settings.is_locked = addon_data.utils.IsChecked(self)
    character_target_settings.is_locked = addon_data.utils.IsChecked(self)
    character_hunter_settings.is_locked = addon_data.utils.IsChecked(self)
