...
addon_data.player.LoadSettings = function()
    ...
    addon_data.utils.NormalizeSettings(character_player_settings, addon_data.player.default_settings)
    -- Update settings that dont change unless the interface is reloaded
    addon_data.player.UpdateInfo()
end

-- On 3.3.5 UnitGUID("player") is not reliably populated as early as
-- ADDON_LOADED, which left player.guid nil so no combat log line ever matched
-- the player. Core also calls this on PLAYER_ENTERING_WORLD.
...
-- 3.3.5's combat log carries no isOffHand flag, so when it's missing attribute
-- the swing to whichever hand was actually due - the timer nearest expiry.
addon_data.player.GuessOffhandSwing = function()
    if not addon_data.player.has_offhand then
        return false
    end
    return addon_data.player.off_swing_timer < addon_data.player.main_swing_timer
end

addon_data.player.OnCombatLogUnfiltered = function(combat_info)
    -- Guard against a GUID that wasn't available yet at load time.
    if addon_data.player.guid == nil then
        addon_data.player.guid = UnitGUID("player")
    end
    -- Index directly rather than unpack(): the normalized table can contain nil
    -- holes on 3.3.5, which would truncate an unpack().
    local event = combat_info[2]
    local source_guid = combat_info[4]
    local spell_name = combat_info[13]
    if (source_guid == addon_data.player.guid) then
        if (event == "SWING_DAMAGE") then
            local is_offhand = combat_info[21]
            if is_offhand == nil then
                is_offhand = addon_data.player.GuessOffhandSwing()
            end
            if is_offhand then
                addon_data.player.ResetOffSwingTimer()
            else
        ...
        elseif (event == "SWING_MISSED") then
            local miss_type = combat_info[12]
            local is_offhand = combat_info[13]
            if is_offhand == nil then
                is_offhand = addon_data.player.GuessOffhandSwing()
            end
            addon_data.core.MissHandler("player", miss_type, is_offhand)
        elseif (event == "SPELL_DAMAGE") or (event == "SPELL_MISSED") then
            local spell_id = combat_info[12]
            if type(spell_id) ~= "number" then
                local _, _, resolved = addon_data.utils.GetSpellInfoCompat(spell_name)
                spell_id = resolved
            end
...
addon_data.player.UpdateVisualsOnUpdate = function()
    ...
    if settings.enabled then
        ...
        frame.main_bar:SetWidth(addon_data.utils.SnapToPixel(main_width))
        ...
        if addon_data.player.has_offhand and settings.show_offhand then
            ...
            frame.off_bar:SetWidth(addon_data.utils.SnapToPixel(off_width))
        ...
        if addon_data.player.has_offhand and character_player_settings.show_offhand then
            frame:SetHeight(addon_data.utils.SnapToPixel((settings.height * 2) + 2))
        else
            frame:SetHeight(addon_data.utils.SnapToPixel(settings.height))
        end
        -- Update the alpha
...
addon_data.player.UpdateVisualsOnSettingsChange = function()
    ...
    if settings.enabled then
        ...
        frame:SetWidth(addon_data.utils.SnapToPixel(settings.width))
        ...
        frame.main_bar:SetHeight(addon_data.utils.SnapToPixel(settings.height))
        if settings.classic_bars then
            frame.main_bar:SetTexture('Interface\\AddOns\\WeaponSwingTimer\\Images\\Bar.blp')
        else
            frame.main_bar:SetTexture('Interface\\AddOns\\WeaponSwingTimer\\Images\\Background.blp')
        end
        frame.main_bar:SetVertexColor(settings.main_r, settings.main_g, settings.main_b, settings.main_a)
        frame.main_spark:SetSize(16, addon_data.utils.SnapToPixel(settings.height))
        ...
        frame.off_bar:SetHeight(addon_data.utils.SnapToPixel(settings.height))
        if settings.classic_bars then
            frame.off_bar:SetTexture('Interface\\AddOns\\WeaponSwingTimer\\Images\\Bar.blp')
        else
            frame.off_bar:SetTexture('Interface\\AddOns\\WeaponSwingTimer\\Images\\Background.blp')
        end
        frame.off_bar:SetVertexColor(settings.off_r, settings.off_g, settings.off_b, settings.off_a)
        frame.off_spark:SetSize(16, addon_data.utils.SnapToPixel(settings.height))
...
addon_data.player.InitializeVisuals = function()
    ...
    frame.main_spark:SetTexture('Interface\\AddOns\\WeaponSwingTimer\\Images\\Spark.blp')
    ...
    frame.off_spark:SetTexture('Interface\\AddOns\\WeaponSwingTimer\\Images\\Spark.blp')
...
addon_data.player.EnabledCheckBoxOnClick = function(self)
    character_player_settings.enabled = addon_data.utils.IsChecked(self)
    addon_data.player.UpdateVisualsOnSettingsChange()
end

addon_data.player.ShowOffHandCheckBoxOnClick = function(self)
    character_player_settings.show_offhand = addon_data.utils.IsChecked(self)
    addon_data.player.UpdateVisualsOnSettingsChange()
end

addon_data.player.ShowBorderCheckBoxOnClick = function(self)
    character_player_settings.show_border = addon_data.utils.IsChecked(self)
    addon_data.player.UpdateVisualsOnSettingsChange()
end

addon_data.player.ClassicBarsCheckBoxOnClick = function(self)
    character_player_settings.classic_bars = addon_data.utils.IsChecked(self)
    addon_data.player.UpdateVisualsOnSettingsChange()
end

addon_data.player.FillEmptyCheckBoxOnClick = function(self)
    character_player_settings.fill_empty = addon_data.utils.IsChecked(self)
    addon_data.player.UpdateVisualsOnSettingsChange()
end

addon_data.player.ShowLeftTextCheckBoxOnClick = function(self)
    character_player_settings.show_left_text = addon_data.utils.IsChecked(self)
    addon_data.player.UpdateVisualsOnSettingsChange()
end

addon_data.player.ShowRightTextCheckBoxOnClick = function(self)
    character_player_settings.show_right_text = addon_data.utils.IsChecked(self)
    addon_data.player.UpdateVisualsOnSettingsChange()
end
...
addon_data.player.MainColorPickerOnClick = function()
    ...
    local function MainOnActionFunc(restore)
        ...
        addon_data.player.frame.main_bar:SetVertexColor(
            settings.main_r, settings.main_g, settings.main_b, settings.main_a)
        addon_data.player.config_frame.main_color_picker.foreground:SetTexture(
            settings.main_r, settings.main_g, settings.main_b, settings.main_a)
    end
...
addon_data.player.MainTextColorPickerOnClick = function()
    ...
    local function MainTextOnActionFunc(restore)
        ...
        addon_data.player.frame.main_right_text:SetTextColor(
            settings.main_text_r, settings.main_text_g, settings.main_text_b, settings.main_text_a)
        addon_data.player.config_frame.main_text_color_picker.foreground:SetTexture(
            settings.main_text_r, settings.main_text_g, settings.main_text_b, settings.main_text_a)
    end
    ColorPickerFrame.func, ColorPickerFrame.opacityFunc, ColorPickerFrame.cancelFunc = 
...
addon_data.player.OffColorPickerOnClick = function()
    ...
    local function OffOnActionFunc(restore)
        ...
        addon_data.player.frame.off_bar:SetVertexColor(
            settings.off_r, settings.off_g, settings.off_b, settings.off_a)
        addon_data.player.config_frame.off_color_picker.foreground:SetTexture(
            settings.off_r, settings.off_g, settings.off_b, settings.off_a)
    end
    ColorPickerFrame.func, ColorPickerFrame.opacityFunc, ColorPickerFrame.cancelFunc = 
...
addon_data.player.OffTextColorPickerOnClick = function()
    ...
    local function OffTextOnActionFunc(restore)
        ...
        addon_data.player.frame.off_right_text:SetTextColor(
            settings.off_text_r, settings.off_text_g, settings.off_text_b, settings.off_text_a)
        addon_data.player.config_frame.off_text_color_picker.foreground:SetTexture(
            settings.off_text_r, settings.off_text_g, settings.off_text_b, settings.off_text_a)
    end
    ColorPickerFrame.func, ColorPickerFrame.opacityFunc, ColorPickerFrame.cancelFunc = 
