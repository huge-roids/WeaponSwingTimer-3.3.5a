...
-- On 3.3.5 the spellcast events give us a spell NAME instead of a spell ID, so
-- build the reverse lookup Core uses to translate one into the other. Ranks of
-- the same spell share a name; the first id wins, which is fine because every
-- is_spell_* check below treats all ranks of a spell identically.
for spell_id, spell_table in pairs(addon_data.hunter.shot_spell_ids) do
    local spell_name = spell_table.spell_name
    if spell_name and addon_data.utils.name_to_spell_id[spell_name] == nil then
        addon_data.utils.name_to_spell_id[spell_name] = spell_id
    end
end
...
addon_data.hunter.OnUseAction = function(action_id)
    ...
    local name, cast_time, real_spell_id = addon_data.utils.GetSpellInfoCompat(WSTScanTipTextLeft1:GetText())
...
addon_data.hunter.OnCastSpellByName = function(name, on_self)
    local cast_time, real_spell_id
    name, cast_time, real_spell_id = addon_data.utils.GetSpellInfoCompat(name)
...
addon_data.hunter.OnCastSpell = function(spell_id, spell_book_type)
    local name, cast_time, real_spell_id = addon_data.utils.GetSpellInfoCompat(spell_id)
    if not addon_data.hunter.casting then
        addon_data.hunter.StartCastingSpell(real_spell_id)
    end
end
...
addon_data.hunter.StartCastingSpell = function(spell_id)
    ...
    if (GetTime() - addon_data.hunter.last_failed_time) > 0 then
        if not addon_data.hunter.casting and UnitCanAttack('player', 'target') then
            local spell_name, cast_time = addon_data.utils.GetSpellInfoCompat(spell_id)
...
addon_data.hunter.LoadSettings = function()
    ...
    addon_data.utils.NormalizeSettings(character_hunter_settings, addon_data.hunter.default_settings)
...
addon_data.hunter.UpdateInfo = function()
    ...
    addon_data.hunter.class = select(2, UnitClass("player"))
...
addon_data.hunter.UpdateVisualsOnUpdate = function()
	...
	if settings.enabled then
        ...
        if not settings.one_bar then
            if addon_data.hunter.auto_shot_ready then
                ...
                if settings.show_multishot_clip_bar then
                    ...
                    -- Without this the clip bar stays visible as a stray red dash.
                    frame.multishot_clip_bar:Hide()
                end
            end
            -- The bar is anchored centre ("TOP"), so clamping a zero width up to
            -- 2px leaves a stray sliver in the middle of the frame whenever the
            -- shot is ready and idle. Hide it instead of drawing the sliver.
            if new_width < 2 then
                new_width = 2
                frame.shot_bar:Hide()
            else
            ...
            frame.shot_bar:SetWidth(addon_data.utils.SnapToPixel(math.min(new_width, settings.width)))
        else
            ...
            if timer_width < 2 then
                timer_width = 2
                frame.shot_bar:Hide()
            else
                frame.shot_bar:Show()
            end
            frame.shot_bar:SetWidth(addon_data.utils.SnapToPixel(math.min(timer_width, settings.width)))
        ...
        if addon_data.hunter.casting_shot then
            ...
            frame:SetSize(addon_data.utils.SnapToPixel(settings.width), addon_data.utils.SnapToPixel((settings.height * 2) + 2))
            ...
            if new_alpha <= 0 then
                new_alpha = 0
                frame:SetSize(addon_data.utils.SnapToPixel(settings.width), addon_data.utils.SnapToPixel(settings.height))
	...
	if settings.enabled then
        ...
        if settings.show_border then
            frame.backplane:SetBackdrop({
                bgFile = "Interface\\AddOns\\WeaponSwingTimer\\Images\\Background.blp", 
                edgeFile = "Interface\\AddOns\\WeaponSwingTimer\\Images\\Border.blp", 
        ...
        if not settings.one_bar then
            ...
            frame.auto_shot_cast_bar:SetHeight(addon_data.utils.SnapToPixel(settings.height))
        ...
        frame.shot_bar:SetHeight(addon_data.utils.SnapToPixel(settings.height))
        if settings.classic_bars then
            frame.shot_bar:SetTexture('Interface\\AddOns\\WeaponSwingTimer\\Images\\Bar.blp')
            frame.auto_shot_cast_bar:SetTexture('Interface\\AddOns\\WeaponSwingTimer\\Images\\Bar.blp')
        else
            frame.shot_bar:SetTexture('Interface\\AddOns\\WeaponSwingTimer\\Images\\Background.blp')
            frame.auto_shot_cast_bar:SetTexture('Interface\\AddOns\\WeaponSwingTimer\\Images\\Background.blp')
        end
        ...
        frame.multishot_clip_bar:SetHeight(addon_data.utils.SnapToPixel(settings.height))
        frame.multishot_clip_bar:SetTexture(settings.clip_r, settings.clip_g, settings.clip_b, settings.clip_a)
        frame.spell_bar:SetPoint("BOTTOMLEFT", 0, 0)
        frame.spell_bar:SetHeight(addon_data.utils.SnapToPixel(settings.height))
        if settings.classic_bars then
            frame.spell_bar:SetTexture('Interface\\AddOns\\WeaponSwingTimer\\Images\\Bar.blp')
        else
            frame.spell_bar:SetTexture('Interface\\AddOns\\WeaponSwingTimer\\Images\\Background.blp')
        end
        frame.spell_spark:SetSize(16, addon_data.utils.SnapToPixel(settings.height))
        frame.spell_text_center:SetPoint("CENTER", 2, -(settings.height / 2))
        frame.cast_latency:SetHeight(addon_data.utils.SnapToPixel(settings.height))
        frame.cast_latency:SetPoint("BOTTOMLEFT", 0, 0)
        frame.cast_latency:SetTexture(1, 0, 0, 0.75)
        if settings.show_latency_bars then
            frame.cast_latency:Show()
        else
            frame.cast_latency:Hide()
...
addon_data.hunter.InitializeVisuals = function()
    ...
    frame.spell_spark:SetTexture('Interface\\AddOns\\WeaponSwingTimer\\Images\\Spark.blp')
...
addon_data.hunter.UpdateConfigPanelValues = function()
    ...
    panel.cooldown_color_picker.foreground:SetTexture(
        settings.cooldown_r, settings.cooldown_g, settings.cooldown_b, settings.cooldown_a)
    panel.autoshot_cast_color_picker.foreground:SetTexture(
        settings.auto_cast_r, settings.auto_cast_g, settings.auto_cast_b, settings.auto_cast_a)
    panel.multi_clip_color_picker.foreground:SetTexture(
        settings.clip_r, settings.clip_g, settings.clip_b, settings.clip_a)
        
    if settings.one_bar then
        panel.explaination:SetTexture('Interface\\AddOns\\WeaponSwingTimer\\Images\\HunterOneBarExplainedAlpha.blp')
        panel.explaination:SetSize(350, 175)
        panel.explaination:SetPoint('TOPLEFT', -50, -385)
    else
        panel.explaination:SetTexture('Interface\\AddOns\\WeaponSwingTimer\\Images\\HunterBarExplainedFullAlpha.blp')
...
addon_data.hunter.EnabledCheckBoxOnClick = function(self)
    character_hunter_settings.enabled = addon_data.utils.IsChecked(self)
    addon_data.hunter.UpdateVisualsOnSettingsChange()
end

addon_data.hunter.ShowAimedShotCastBarCheckBoxOnClick = function(self)
    character_hunter_settings.show_aimedshot_cast_bar = addon_data.utils.IsChecked(self)
    addon_data.hunter.UpdateVisualsOnSettingsChange()
end

addon_data.hunter.ShowMultiShotCastBarCheckBoxOnClick = function(self)
    character_hunter_settings.show_multishot_cast_bar = addon_data.utils.IsChecked(self)
    addon_data.hunter.UpdateVisualsOnSettingsChange()
end

addon_data.hunter.ShowLatencyBarsCheckBoxOnClick = function(self)
    character_hunter_settings.show_latency_bars = addon_data.utils.IsChecked(self)
    addon_data.hunter.UpdateVisualsOnSettingsChange()
end

addon_data.hunter.ShowMultiShotClipBarCheckBoxOnClick = function(self)
   character_hunter_settings.show_multishot_clip_bar = addon_data.utils.IsChecked(self)
    addon_data.hunter.UpdateVisualsOnSettingsChange()
end

addon_data.hunter.ShowBorderCheckBoxOnClick = function(self)
    character_hunter_settings.show_border = addon_data.utils.IsChecked(self)
    addon_data.hunter.UpdateVisualsOnSettingsChange()
end

addon_data.hunter.ClassicBarsCheckBoxOnClick = function(self)
    character_hunter_settings.classic_bars = addon_data.utils.IsChecked(self)
    addon_data.hunter.UpdateVisualsOnSettingsChange()
end

addon_data.hunter.OneBarCheckBoxOnClick = function(self)
    character_hunter_settings.one_bar = addon_data.utils.IsChecked(self)
    addon_data.hunter.UpdateVisualsOnSettingsChange()
    addon_data.hunter.UpdateConfigPanelValues()
end

addon_data.hunter.ShowTextCheckBoxOnClick = function(self)
    character_hunter_settings.show_text = addon_data.utils.IsChecked(self)
    addon_data.hunter.UpdateVisualsOnSettingsChange()
end
...
addon_data.hunter.CooldownColorPickerOnClick = function()
    ...
    local function CooldownOnActionFunc(restore)
        ...
        addon_data.hunter.config_frame.cooldown_color_picker.foreground:SetTexture(
            settings.cooldown_r, settings.cooldown_g, settings.cooldown_b, settings.cooldown_a)
        addon_data.hunter.UpdateVisualsOnSettingsChange()
    end
...
addon_data.hunter.AutoShotCastColorPickerOnClick = function()
    ...
    local function AutoShotCastOnActionFunc(restore)
        ...
        addon_data.hunter.config_frame.autoshot_cast_color_picker.foreground:SetTexture(
            settings.auto_cast_r, settings.auto_cast_g, settings.auto_cast_b, settings.auto_cast_a)
        addon_data.hunter.UpdateVisualsOnSettingsChange()
    end
    ColorPickerFrame.func, ColorPickerFrame.opacityFunc, ColorPickerFrame.cancelFunc = 
...
addon_data.hunter.MultiClipColorPickerOnClick = function()
    ...
    local function MultiClipOnActionFunc(restore)
        ...
        addon_data.hunter.frame.multishot_clip_bar:SetTexture(settings.clip_r, settings.clip_g, settings.clip_b, settings.clip_a)
        addon_data.hunter.config_frame.multi_clip_color_picker.foreground:SetTexture(
            settings.clip_r, settings.clip_g, settings.clip_b, settings.clip_a)
    end
    ColorPickerFrame.func, ColorPickerFrame.opacityFunc, ColorPickerFrame.cancelFunc = 
